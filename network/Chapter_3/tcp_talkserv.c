...
int recv_and_print(int sd); // 상대로부터 메시지 수신후 화면출력, 자식 프로세스에서 수행
int input_and_send(int sd); // 키보드입력받고 상대에게 메시지전달, 부모 프로세스에서 수행

#define MAXLINE 511

int main(int argc, char *argv[]) {
    struct sockaddr_in cliaddr, servaddr;
    int listen_sock, accp_sock, addrlen = sizeof(cliaddr); // 소켓번호, 주소길이
    pid_t pid;
    if (argc != 2) {
        printf("사용법: %s port\n", argv[0]);
        exit(0);
